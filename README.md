...
Open the extension popup to choose blur, full black, or face-only mode. Face-only mode activates automatically for every participant camera. Blur and full black remain per-participant: hover over a camera tile and click the eye button to hide or show it.
...
- Face-only mode activates automatically and requires three stable face detections before showing a crop. A sparse optical-flow tracker and Kalman filter follow short head movements when the face is covered; the complete video returns if both detection and motion tracking remain unavailable or occlusion exceeds five seconds.
